...
void mutateSquare(vector<vector<int>>& square) {
    ...
    while (mutationCount > 0) {
        ...
        if (square[i1][j1] != square[i2][j2]) { // Avoid unnecessary swaps
...
vector<vector<int>> findMagicSquare(int n, int populationSize, int maxIterations) {
...
>>>>>>> Stashed changes
        ...
        if (minFitness == 0) {
            ...
            cout << "Found optimal solution, breaking early" << endl;
            break;
        }
...
=======
    ...
    vector<vector<int>> magicSquare = findMagicSquare(n, populationSize, maxIterations);

    cout << "Magic Square:" << endl;
    printSquare(magicSquare);

    cout << "Fitness = " << calculateFitness(magicSquare) << endl;
