...
/** Important functions **/
void run_experiment(register int matrix_dimension, register int power);
void initialize_matrix(MatrixInitialization init_type, register int dimension);
void raise_to_power(register int dimension, register int power);
int multiply_matrix(register int dimension);
...
void run_experiment(register int matrix_dimension, register int power)
{
	for(int i = 0; i < 30; i++)
	{
		...
		raise_to_power(matrix_dimension, power);
...
void initialize_matrix(MatrixInitialization init_type, register int dimension)
{
	for(register int i = 0; i < dimension; i++)
	{
		...
		{
			register int matrix_index = i * dimension + j;
			switch(init_type)
			{
			case Identity:
...
void raise_to_power(register int dimension, register int power)
{
	for(register int i = 0; i < power; i++)
	{
...
int multiply_matrix(register int dimension)
{
	register int all_nan = 1;
	for(register int i = 0; i < dimension; i++)
	{
		for(register int j = 0; j < dimension; j++)
		{
			register int matrix_index = i * dimension + j;
			register MATRIX_TYPE temp = 0.0;
			register MATRIX_TYPE left = 0.0;
			register MATRIX_TYPE right = 0.0;
			
			for(register int k = 0; k < dimension; k++)
			{
				left = previous[i * dimension + k];
				right = matrix[k * dimension + j];
				temp += left * right;
			}
			current[matrix_index] = temp;
